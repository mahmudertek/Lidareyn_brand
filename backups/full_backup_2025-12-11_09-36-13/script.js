// ============================================
// RESELS - MODERN E-COMMERCE JAVASCRIPT
// ============================================
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initLiveSearch() {
        console.log('Live Search: Initializing inside script.js...');

        const searchInput = document.querySelector('.search-input');
        // Flexible container selection
        const searchContainer = document.querySelector('.search-container') || (searchInput ? searchInput.parentElement : null);
        ...
        if (!document.getElementById('live-search-style')) {
            ...
            style.innerHTML = `
                ...
                .search-result-product img {
                    width: 40px; height: 50px; object-fit: cover; border-radius: 4px; border: 1px solid #eee;
                }
                .search-result-info { display: flex; flex-direction: column; }
                .search-result-name { font-size: 0.9rem; font-weight: 500; }
                .search-result-price { font-size: 0.85rem; font-weight: 700; color: #8b7bd8; }
                .search-icon-circle {
                    width: 32px; height: 32px; border-radius: 50%; background: #eee;
                    display: flex; align-items: center; justify-content: center; color: #666;
                }
                .no-results { padding: 15px; text-align: center; color: #888; }
            `;
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            const matchedBrands = ['Adidas', 'Nike', 'Puma', 'Zara', 'Mavi', 'Defacto', 'Samsung', 'Apple']
                .filter(b => b.toLowerCase().includes(query)).slice(0, 3);

            const matchedCategories = [...new Set(products.map(p => p.category.split(' > ')[0]))]
                .filter(c => c.toLowerCase().includes(query)).slice(0, 2);
            ...
            if (matchedCategories.length > 0) {
                ...
                matchedCategories.forEach(cat => {
                    html += `
                    <a href="arama.html?category=${encodeURIComponent(cat)}" class="search-result-item">
            ...
            if (matchedBrands.length > 0) {
                ...
                matchedBrands.forEach(brand => {
                    html += `
                    <a href="arama.html?q=${encodeURIComponent(brand)}" class="search-result-item">
            ...
            if (matchedProducts.length > 0) {
                ...
                matchedProducts.forEach(prod => {
                    html += `
                    <a href="urun-detay.html?id=${prod.id}" class="search-result-item search-result-product">
            ...
            if (html === '') {
                html = `<div class="no-results">"${query}" bulunamadı.</div>`;
            }
    ...
    document.querySelectorAll('img').forEach(img => {
        img.addEventListener('error', function () {
            ...
            this.src = 'https://picsum.photos/seed/resels-placeholder/300x200.jpg';
            this.alt = 'Görsel yüklenemedi - Resels e-ticaret';
