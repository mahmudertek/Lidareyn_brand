...
# 1. Define Categories Data Cleaned (Source of Truth)
categories_data = {
...
# Pattern
pattern = r'(<ul class="mega-menu-list">[\s\S]*?</ul>)'
match = re.search(pattern, content)

if match:
    # Look for the SECOND occurence? No, the mega menu list is usually unique or first
    # In index.html, the mega menu is inside <div class="mega-menu">
    # Let's ensure we match the right one
    
    # We replace the entire <ul class="mega-menu-list">...</ul> block
    new_content = content.replace(match.group(0), html_output)
    
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully rebuilt Mega Menu in index.html with CLEAN data.")
else:
    print("Could not find <ul class=\"mega-menu-list\"> in index.html")
