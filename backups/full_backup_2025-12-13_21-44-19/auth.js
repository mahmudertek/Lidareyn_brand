...
async function apiCall(endpoint, method = 'GET', data = null) {
    ...
    // Add token if exists
    const token = localStorage.getItem('karakoytuccari_token');
    if (token) {
...
window.Auth = {
    ...
    register: async (email, password, name, gender) => {
        try {
            ...
            if (result.success) {
                ...
                sessionStorage.setItem('karakoytuccari_pending_email', email);
                return { success: true };
            } else {
    ...
    verify: async (inputCode) => {
        try {
            const email = sessionStorage.getItem('karakoytuccari_pending_email');
            if (!email) {
            ...
            if (result.success) {
                // Store token
                localStorage.setItem('karakoytuccari_token', result.token);
                localStorage.setItem('karakoytuccari_current_user', JSON.stringify(result.user));

                // Clear pending email
                sessionStorage.removeItem('karakoytuccari_pending_email');
    ...
    // Get current user
    getCurrentUser: () => {
        const userStr = localStorage.getItem('karakoytuccari_current_user');
    ...
    getMe: async () => {
        try {
            ...
            if (result.success) {
                localStorage.setItem('karakoytuccari_current_user', JSON.stringify(result.data));
                return result.data;
            }
            return null;
        } catch (error) {
