...
function initLiveSearch() {
    ...
    if (searchBtn) {
        searchBtn.addEventListener('click', (e) => {
            ...
            if (searchContainer.classList.contains('active')) {
                // If input is empty, close it. If has text, maybe submit? (For now just close if empty)
                if (searchInput.value.trim() === '') {
                    searchContainer.classList.remove('active');
                } else {
                    // Trigger search or keep open logic logic
                    console.log('Search submit or already open');
                }
            } else {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        // 1. Products (Search ONLY by name - more precise)
