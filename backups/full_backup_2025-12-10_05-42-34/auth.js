...
window.Auth = {
    ...
    sendVerificationEmail: (email, name, code) => {
        // DEBUG: Mail gecikmelerine karşı kodu ekrana yansıtıyoruz.
        // İleride site yayına girdiğinde bu alert satırını kaldırabilirsiniz.
        alert(`[DEBUG] Gmail Gecikmesi İçin Geçici Kod: ${code}\n\nTamam'a basın, gerçek mail de yolda.`);
        ...
        if (window.emailjs) {
            ...
            emailjs.send(EMAIL_CONFIG.SERVICE_ID, EMAIL_CONFIG.TEMPLATE_ID, {
                ...
                .then(function () {
                    ...
                    // alert(`Doğrulama kodu ${email} adresine gönderildi.`); // Debug alert varken buna gerek yok, kullanıcıyı boğmayalım.
                }, function (error) {
                    console.log('FAILED...', error);
                    // Silent fail because we already gave the code in debug alert
                });
        }
        // 2. Fallback
        else {
