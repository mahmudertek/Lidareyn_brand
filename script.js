...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const performSearch = () => {
        if (elements.searchInput) {
            ...
            if (query) {
                // Sayfa konumuna göre arama sayfası yolunu belirle
                const isInSubfolder = window.location.pathname.includes('/kategoriler/');
                const searchPagePath = isInSubfolder ? '../arama.html' : 'arama.html';
    ...
    // Open Favorites (Global Listener for all pages)
    if (elements.favoritesBtn) {
        elements.favoritesBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            // Giriş kontrolü (opsiyonel - isterseniz kaldırabilirsiniz)
    ...
    window.toggleFavorite = function (product) {
        ...
        // Sayfa yenilemeye gerek yok, badge güncellense yeter
        // if(window.updateFavoriteCount) window.updateFavoriteCount();
    };

    // Navigate to Cart Page (Global Listener for all pages)
    if (elements.cartBtn) {
